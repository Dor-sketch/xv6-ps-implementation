/* an edited proc.c file with the new system call function.
  the system call function was defined here according to the instructions.*/
#include "types.h"
#include "defs.h"
#include "param.h"
#include "memlayout.h"
...
int cps192(void)
{
  ...
  for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
    if ( p->state == SLEEPING )
      cprintf("%s \t %d \t SLEEPING \t %d \t \n ", p->name, p->pid, (p->pid == 1) ? 1 : p->parent->pid);
